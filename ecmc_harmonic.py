...
for Iter in range(n_trials):
    ...
    while NData < n_steps:
        ...
        x_minus = x[k_minus] 
        if k == 0: x_minus -= L
        Upsilon_plus = -2.0 * math.log(random.uniform(0.0, 1.0))
        Upsilon_minus = -2.0 * math.log(random.uniform(0.0, 1.0))
        z_plus = x_plus - b 
        z_minus = x_minus + b
#
#   forward factor
#
        if x[k] < z_plus: Del_plus = (z_plus - x[k]) + math.sqrt(Upsilon_plus)
        else: Del_plus = math.sqrt(Upsilon_plus + (x[k] - z_plus) ** 2) + z_plus - x[k]
#
#   backward factor
#
        if x[k] < z_minus: Del_minus = (z_minus - x[k]) + math.sqrt(Upsilon_minus)
        else: Del_minus = math.sqrt(Upsilon_minus + (x[k] - z_minus) ** 2) + z_minus - x[k]
